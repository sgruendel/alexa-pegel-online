...
export function parseDialogOutput(output, expectedTurns) {
    ...
    for (const invocation of output.invocations ?? []) {
        ...
        if (body && body.status !== 'IN_PROGRESS') completed.set(body.id ?? completed.size, body);
    }
    const turns = [...completed.values()];
    for (const [index, turn] of turns.entries()) {
        const error = turn.result?.error;
        if (error) throw new SimulationError(`Turn ${index + 1}: ${error.message}`, error.message === 'An unexpected error occurred.');
        if (turn.status !== 'SUCCESSFUL') throw new SimulationError(`Turn ${index + 1}: simulation status ${turn.status}`);
        const speech = turn.result?.alexaExecutionInfo?.alexaResponses;
...
export async function runDialog(replayFile, {
    ...
    const directory = await mkdtemp(path.join(tmpdir(), 'alexa-pegel-dialog-'));
